        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-opacity-50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6 space-y-6">
                    <div>
                        <p className="text-lg text-gray-500">Record Date</p>
                            ...
                            {Object.entries(readings || {}).map(([key, readingObject]) => {
                                ...
                                return (
                                    <div key={key} className="bg-gray-50 rounded-lg p-4">
                                        <p className="text-lg text-gray-600 mb-1">{readingObject.testName || field?.label || key}</p>
                                        <p className="text-xl font-bold text-gray-900">
                                            {readingObject.value} <span>   </span>
                                            <span className="text-lg text-gray-500">{readingObject.unit || field?.unit || ''}</span>
                                        </p>
                                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-lg font-medium mt-1
